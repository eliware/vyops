...
const { validateRemoteScript } = await import('../src/backup.mjs');

beforeEach(() => exec.mockReset());

test('accepts a stable regular remote script', async () => {
  exec.mockResolvedValue({ code: 0, stdout: '', stderr: '' });
  await expect(validateRemoteScript({}, '/config/scripts/hook.sh')).resolves.toBeUndefined();
...
test('rejects a remote script whose metadata check fails', async () => {
  ...
  await expect(validateRemoteScript({}, '/config/scripts/hook.sh'))
